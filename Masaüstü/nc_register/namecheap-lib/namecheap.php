...
// *                                                                          *
// * Copyright 2008-2011 NameCheap.com                                        *
// *                                                                          *
...
// * namecheap.php, namecheapapi.php and namecheapsync.php into it.           *
...
// * - Registration and extended attributes for .asia
// * NOTE: at this time WHMCS doesn't support extended attributes for .asia domains. You have to add below code at the end
// * of the file includes/additionaldomainfields.php before ? >
// *    $additionaldomainfields[".asia"][] = array(
// *      "Name" => "Locality of Establishment of the Entity",
// *      "Type" => "dropdown",
...
// *    );
// *    $additionaldomainfields[".asia"][] = array(
// *      "Name" => "Form of Identity",
// *      "Type" => "dropdown",
// *      "Options" => "certificate,legislation,societyRegistry,politicalPartyRegistry,passport,other"
// *    );
// *    $additionaldomainfields[".asia"][] = array(
// *      "Name" => "Form of Identity ID Number/Code of Reference",
// *      "Type" => "text",
// *      "Size" => "20",
// *      "Default" => "",
// *      "Required" => true,
...
function namecheap_SaveNameservers($params)
{
    ...
    $nameservers = array($params['ns1'], $params['ns2'], $params['ns3'], $params['ns4']);
...
function namecheap_SaveRegistrarLock($params)
{
    ...
    try
    {
        ...
        $rl_unable_domains = array("ca", "cm", "co.uk", "org.uk", "de", "eu", "ws");
...
function namecheap_RegisterDomain($params)
{
    ...
    try
    {
        ...
        if ('eu' == strtolower($tld)) { // for .eu domains
        ...
        } elseif ('co.uk' == strtolower($tld) || 'org.uk' == strtolower($tld)) {
            ...
            $request_params['ASIACCLocality'] = $params['additionalfields']['Locality of Establishment of the Entity'];
            $request_params['ASIALegalEntityType'] = $params['additionalfields']['Type of Entity'];
            $request_params['ASIAIdentForm'] = $params['additionalfields']['Form of Identity'];
            $request_params['ASIAIdentNumber'] = $params['additionalfields']['Form of Identity ID Number/Code of Reference'];
        }
...
function namecheap_GetContactDetails($params)
{
    ...
    try
    {
        ...
        $wg_unable_domains = array("bz", "ca", "cn", "co.uk", "de", "eu", "in", "me.uk", "mobi", "nu", "org.uk", "us", "ws");

        // we can't set $values['error'], because WHMCS returns only error message and not any data
        $warn = '<div class="infobox"><br />%s<br /><br /></div>';
        if (in_array(strtolower($tld), $wg_unable_domains)) {
            if (defined('CLIENTAREA') && CLIENTAREA == true) {
                $values[sprintf($warn, "Contact information can not be protected for this domain type")] = array();
            } else {
                $values[sprintf($warn, "Whoisguard can not be used with this TLD due to registry restrictions")] = array();
            }
        } elseif (isset($result['DomainContactsResult']['WhoisGuardContact'])) {
            if (defined('CLIENTAREA') && CLIENTAREA == true) {
                $values[sprintf($warn, 'Your contact information is protected, changes will be internal only')] = array();
            } else {
                $values[sprintf($warn, 'Whoisguard is enabled for this domain, contact information changes will be internal only')] = array();
            }
        } else {
            if (defined('CLIENTAREA') && CLIENTAREA == true) {
                //$values[sprintf($warn, 'Whoisguard is disabled, please contact support to enable it')] = array();
                $values[sprintf($warn, 'Your contact information is not protected')] = array();
            } else {
                $values[sprintf($warn, 'Whoisguard is disabled, it is not possible to enable it via API')] = array();
            }
        }
        ...
        foreach ($result['DomainContactsResult'] as $k => $v) {
            ...
            $values[$k]['FirstName']        = $v['FirstName'];
            $values[$k]['LastName']         = $v['LastName'];
            $values[$k]['OrganizationName'] = $v['OrganizationName'];
            $values[$k]['Address1']         = $v['Address1'];
            $values[$k]['Address2']         = $v['Address2'];
            $values[$k]['City']             = $v['City'];
            $values[$k]['StateProvince']    = $v['StateProvince'];
            $values[$k]['PostalCode']       = $v['PostalCode'];
            $values[$k]['Country']          = $v['Country'];
            $values[$k]['Phone']            = $v['Phone'];
            $values[$k]['Fax']              = $v['Fax'];
            $values[$k]['EmailAddress']     = $v['EmailAddress'];
        }
        $attr = $result['DomainContactsResult']['CurrentAttributes'];
        if (!empty($attr)) {
            if (in_array(strtolower($tld), array("us", "ca", "co.uk", "org.uk"))) {
                if (!defined('CLIENTAREA') || CLIENTAREA != true) {
                    $msg = strtoupper($tld)
                        . ' domains: <em>Please refer to <a href="http://developer.namecheap.com/docs/doku.php?id=api-reference:extended.attributes" target="_blank">API documentation</a> for '
                        . strtoupper($tld) . ' legal types</em>';
                    $values[sprintf($warn, $msg)] = array();
                }
            }
            foreach ($attr as $k => $v) {
                $values['CurrentAttributes'][$k] = $v;
            }
            // for .ca domains
            if ("ca" == strtolower($tld)) {
                //
            }
            // for .co.uk & .org.uk domains
            $key = strtoupper(str_replace('.', '', $tld));
            if (("co.uk" == strtolower($tld) || "org.uk" == strtolower($tld)) && !isset($attr[$key . 'CompanyID'])) {
                $values['CurrentAttributes'][$key . 'CompanyID'] = "";
            }
        }
    }
    catch (Exception $e) {
