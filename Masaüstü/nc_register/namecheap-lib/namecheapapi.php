...
class NamecheapApi
{
    ...
    public function request($command, array $params)
    {
        ...
        if (!$result) {
            throw new NamecheapApiException($curl_error ? $curl_error : "Unable to request data from " . $url);
        }
        return $result;
    }
    ...
    private function _formatPhone($phone)
    {
        ...
        if (!$phone_code) {
            throw new Exception("Invalid phone number or phone country code");
        }
